...
import arcpy, numpy, os, sys, traceback, time, csv
...
class interface(object):
        ...
        if max01 > self.memTh:
            ...
            if maxArea > areaTh:
                ...
                if max02 > areaTh:
                    ...
                    arcpy.AddField_management(tmpPoly, 'OTier1', 'TEXT', 20)
                    arcpy.AddField_management(tmpPoly, 'UTier1', 'TEXT', 20)
                    arcpy.AddField_management(tmpPoly, 'OTier2', 'TEXT', 25)
                    arcpy.AddField_management(tmpPoly, 'UTier2', 'TEXT', 25)
                    arcpy.AddField_management(tmpPoly, 'rasCode', 'SHORT', 2)
                    # Populate fields
                    fields = ['OTier1', 'OTier2', 'rasCode']
                ...
                if saveNQ == 'TRUE':
                    ...
                    # Add Tier 1 and Tier 2 Names to attribute table
                    arcpy.AddField_management(tmpPolyNQ, 'OTier1', 'TEXT', 20)
                    arcpy.AddField_management(tmpPolyNQ, 'UTier1', 'TEXT', 20)
                    arcpy.AddField_management(tmpPolyNQ, 'OTier2', 'TEXT', 25)
                    arcpy.AddField_management(tmpPolyNQ, 'UTier2', 'TEXT', 25)
    ...
    def Tier3(self):
        ...
        tier3Dir = "tier3"
        tier3Path = os.path.join(self.output_directory, tier3Dir)
        self.mkdir(tier3Path)
        arcpy.env.workspace = tier3Path
        ...
        try: 
            ...
            csvCHUnitFieldName = "ChannelUnitNumber"
            chUnitFieldName = "ChannelUnitNumber"
            if hasattr(self, 'champLWD'):
                arcpy.TableToTable_conversion(self.champLWD.path, env.workspace, 'tbl_lw.dbf')
                chUnitFieldName = self.findArcpyFieldNameFromCSV(csvCHUnitFieldName, self.champLWD.path, 'tbl_lw.dbf')
                arcpy.Statistics_analysis('tbl_lw.dbf', 'tbl_lw_sum.dbf', [['SumLWDCoun', 'SUM']], chUnitFieldName)
            ...
            # Count CSV is generated by frequency analysis using Large Woody Piece file CHaMP (2014-present)
                ...
                chUnitFieldName = self.findArcpyFieldNameFromCSV(csvCHUnitFieldName, self.champLWP.path, 'tbl_lw.dbf')
                arcpy.Frequency_analysis('tbl_lw.dbf', 'tbl_lw_sum.dbf', chUnitFieldName)
                arcpy.AddField_management('tbl_lw_sum.dbf', 'lwdCount', 'SHORT')
                arcpy.CalculateField_management('tbl_lw_sum.dbf', 'lwdCount', '[FREQUENCY]')
            else:
                raise Exception('Could not find valid woody debris/piece information. Please check that you specified it.')
            ...
            arcpy.JoinField_management(tmp_cus, 'Unit_Numbe', 'tbl_cover.dbf', chUnitFieldName, 'BouldersGT')
            arcpy.JoinField_management(tmp_cus, 'Unit_Numbe', 'tbl_lw_sum.dbf', chUnitFieldName, 'lwdCount')
            ...
            arcpy.JoinField_management(tmp_cus, 'Unit_Numbe', 'tbl_d84.dbf', chUnitFieldName, 'D84')
            ...
            keepList = ['GRIDCODE', 'OTier1', 'UTier1', 'OTier2', 'UTier2', 'rasCode', 'guArea', 'guLength',
            ...
            fields = ['OTier2', 'guWidth', 'guLength', 'guPosition', 'guOrient', 'lwdCount', 'pBoulder', 'LFRR', 'bfeSlope']
            ...
            fields = ['OTier1','rasCode', 'SHAPE@Area', 'guArea']
    ...
    def findArcpyFieldNameFromCSV(self, csvName, csvFile, ArcpyTable):
        result = csvName
        if len(csvName) < 10:
            return csvName
        with open(csvFile) as f:
            ...
            for i,col in enumerate(row):
                if csvName in col:
                    tablefields = arcpy.ListFields('tbl_lw.dbf')
                    return tablefields[i+1].name # the first field is always OID so we push by one.
        return csvName
