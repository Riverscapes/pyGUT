...
MakeUNITIDboxplots=function(datapath, mycolors="GU.colors", attribute="GU", 
                            extent="suitable", plotoutliers=F){
  ...
  if(extent=="bankfull"){
    dens=data$No.Fish/data$Area #makes density response for Bankfull Area
    #length(s.dens)
  }
  
  if(extent=="suitable"){
   dens=data$No.Fish/data$SuitArea #makes density response for Suitable Area
   #length(s.dens)
  }
   
  if(extent=="best"){
   dens=data$No.FishBest/data$BestArea #makes density response for Best Area
 #  length(b.dens)
}

  if(extent=="model"){  
   dens=data$No.Fish/data$Area#makes density response for Geomorphic Unit
 #  length(u.dens)
  }
  
  attributecol=which(names(data)==attribute)
  unit=as.factor(data[,attributecol])
  
  
  if(length(which(dens=="NaN"))>0){
    unit=unit[-which(dens=="NaN")]
    dens=dens[-which(dens=="NaN")]
  }
  
  if(length(which(dens=="Inf"))>0){
    unit=unit[-which(dens=="Inf")]
    dens=dens[-which(dens=="Inf")]
  }

   n=length(na.omit(dens)) #number of responsis not NA
   print("n=")
   n
     

   
   print("does length of units equal length of response?")
   print(length(unit)==length(dens))
   
   plot.color=mycolors[match(levels(as.factor(unit)), names(mycolors))]
   #plot.color=sort(mycolors)
   

   
   if(plotoutliers==F){
   outliers=which(dens>mean(dens, na.rm=T)+2*sd(dens, na.rm=T))
   dens=dens[-outliers]
   unit=unit[-outliers]
   }
   

   #unit.colors[match(as.factor(w.data$formcolor), names(form.colors))]

#   
#   pdf(paste(figdir, "NoFishpsqm.pdf", sep="//"))
   par(mfrow=c(1,1), mar=c(9,4,3,1))
   boxplot(dens~unit, las=2, col=plot.color, ylab="No.Fish/sq.m", 
           main=paste(extent, "extent", "n=",n ,""))
 }