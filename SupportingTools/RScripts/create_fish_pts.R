...
check.fish.pts = function(data, zrank = "max", plot.nrei = FALSE){
  ...
  if(all(plot.nrei == TRUE & data$all.nrei.pts.csv == 'Yes')){
    ...
    if(n.rows > 0){
      ...
      zrank.raster = pts.zrank %>% 
        mutate(ras.val = ifelse(max.nrei_Jph > 0 & rad.step.gte.user.pval > 0.4, max.nrei_Jph, NA)) %>% 
        dplyr::select(X, Y, ras.val) %>% 
        as.data.frame() %>% 
        dplyr::select(-geometry) %>%
        rasterFromXYZ(crs = crs(pts.zrank))
        # rasterFromXYZ(res = c(0.1, 0.1), crs = crs(pts.zrank))
