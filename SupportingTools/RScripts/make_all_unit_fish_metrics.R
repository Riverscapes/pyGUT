...
#'
#' @examples
make.all.unit.fish.metrics = function(visit.dir, gut.layer, gut.field){
  ...
  # if gut units or extent shp don't exist then return tibble with just visit.id
  # otherwise create tibble with unit id, unit type and unit area to append metrics to
  # rename area units shp field to distinguish from delft area field
  if(any(!"sf" %in% class(units.shp), !"sf" %in% class(extent.shp))){
    out.metrics = tibble(visit.id = visit.id)
    return(out.metrics)
  }else{
    units.shp = units.shp %>%
      mutate(area.unit = round(Area, 3)) %>%
      dplyr::select(-Area)
    tb.metrics = units.shp %>%
      st_drop_geometry() %>%
      dplyr::select(UnitID, !!sym(gut.field), area.unit)
  }
  

  # clip units to extent shp and calculate new area field (area.delft)
  # sum delft area for each unit id and append to tb.metrics 
  if("sf" %in% class(extent.shp)){
    units.clip = units.shp %>% 
      st_intersection(extent.shp) %>%
      mutate(extent.area = st_area(.) %>% as.numeric()) %>% 
      st_drop_geometry() %>%
      group_by(UnitID) %>%
      summarize(area.delft = sum(extent.area, na.rm = TRUE) %>% round(3))
    tb.metrics = tb.metrics %>%
      left_join(units.clip, by = "UnitID") %>%
      mutate(area.delft = ifelse(is.na(area.delft) & !is.na(area.unit), 0.0, area.delft))
  }
  

  # create reference tibbles with just unit id field and suitable categories
  ...
  if(!"sf" %in% class(nrei.all.shp)){
    nrei.metrics.wide = tb.metrics %>%
      mutate(model = "nrei", species = "steelhead", lifestage = "juvenile",
             hab.area.suitable = NA, pred.fish = NA, 
    ...
    nrei.metrics = nrei.metrics %>%
      bind_rows(., calc.unit.area(units.nrei.suit, "UnitID", tb.units, tb.units.cats, by.suit = TRUE) %>% mutate(var = "hab.area"))
    
    # capacity by group field
    nrei.metrics = nrei.metrics %>% 
      bind_rows(., calc.unit.capacity(nrei.locs.shp, "UnitID", tb.units, tb.units.cats, by.suit = FALSE) %>% mutate(var = "pred.fish"))
    
    # model values by group field
    nrei.metrics = nrei.metrics %>% 
      bind_rows(., calc.unit.stats(nrei.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "nre_Jph", by.suit = FALSE))
    
    # model values by group field and suitability category
    nrei.metrics = nrei.metrics %>% 
      bind_rows(., calc.unit.stats(nrei.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "nre_Jph", by.suit = TRUE))
    ...
    nrei.metrics.wide = nrei.metrics %>%
      ...
      right_join(tb.metrics, by = "UnitID") %>%
      mutate(model = "nrei", species = "steelhead", lifestage = "juvenile") %>%
  ...
  if(!"sf" %in% class(ch.all.shp)){
    ...
    ch.metrics = ch.metrics %>% 
      bind_rows(., calc.unit.area(units.ch.suit, "UnitID", tb.units, tb.units.cats, by.suit = TRUE) %>% mutate(var = "hab.area"))
    
    # capacity by group field
    ch.metrics = ch.metrics %>% 
      bind_rows(., calc.unit.capacity(ch.redds.shp, "UnitID", tb.units, tb.units.cats, by.suit = FALSE) %>% mutate(var = "pred.fish"))
    
    # model values by group field
    ch.metrics = ch.metrics %>% 
      bind_rows(., calc.unit.stats(ch.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "ras.value", by.suit = FALSE))
    
    # model values by group field and suitability category
    ch.metrics = ch.metrics %>% 
      bind_rows(., calc.unit.stats(ch.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "ras.value", by.suit = TRUE))
  ...
  if(!"sf" %in% class(st.all.shp)){
    ...
    st.metrics = st.metrics %>% 
      bind_rows(., calc.unit.area(units.st.suit, "UnitID", tb.units, tb.units.cats, by.suit = TRUE) %>% mutate(var = "hab.area"))
    
    # capacity by group field
    st.metrics = st.metrics %>% 
      bind_rows(., calc.unit.capacity(st.redds.shp, "UnitID", tb.units, tb.units.cats, by.suit = FALSE) %>% mutate(var = "pred.fish"))
    
    # model values by group field
    st.metrics = st.metrics %>% 
      bind_rows(., calc.unit.stats(st.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "ras.value", by.suit = FALSE))
    
    # model values by group field and suitability category
    st.metrics = st.metrics %>% 
      bind_rows(., calc.unit.stats(st.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "ras.value", by.suit = TRUE))
  ...
  if(!"sf" %in% class(ch.juv.all.shp)){
    ...
    ch.juv.metrics = ch.juv.metrics %>% 
      bind_rows(., calc.unit.area(units.ch.juv.suit, "UnitID", tb.units, tb.units.cats, by.suit = TRUE) %>% mutate(var = "hab.area"))
    
    # capacity by group field
    ch.juv.metrics = ch.juv.metrics %>% 
      bind_rows(., calc.unit.capacity(ch.locs.shp, "UnitID", tb.units, tb.units.cats, by.suit = FALSE) %>% mutate(var = "pred.fish"))
    
    # model values by group field
    ch.juv.metrics = ch.juv.metrics %>% 
      bind_rows(., calc.unit.stats(ch.juv.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "ras.value", by.suit = FALSE))
    
    # model values by group field and suitability category
    ch.juv.metrics = ch.juv.metrics %>% 
      bind_rows(., calc.unit.stats(ch.juv.all.shp, "UnitID", tb.units, tb.units.cats, in.field = "ras.value", by.suit = TRUE))
  ...
  out.metrics = bind_rows(nrei.metrics.wide, ch.metrics.wide, st.metrics.wide, ch.juv.metrics.wide) %>%
    ...
    dplyr::select(visit.id, UnitID, !!sym(gut.field), area.unit, model, species, lifestage, area.delft, hab.area.suitable, pred.fish, med, mean, sd, med.suitable, mean.suitable, sd.suitable)
